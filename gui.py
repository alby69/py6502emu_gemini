...
from bus import Bus
from cia import C64_KEY_MAP # Import the key mapping
import numpy as np
import sys
import datetime
import imageio
...
class EmulatorGUI:
    ...
    def reset_and_load(self, prg_file=None):
        ...
        try:
            self.bus.load_basic_rom_from_file("c64_basic.rom")
            self.bus.load_kernal_rom_from_file("c64_kernal.rom")
            self.bus.vic.load_char_rom()

            if prg_file and prg_file.lower().endswith('.crt'):
                self.bus.load_crt(prg_file)
                # For cartridges, we just reset. The cartridge ROM will handle startup.
                self.cpu.pc = (self.bus.read(0xFFFC) | (self.bus.read(0xFFFD) << 8))
                print(f"Cartridge loaded. Resetting CPU. PC set to reset vector: ${self.cpu.pc:04X}")
            elif prg_file:
                # Load a .prg file
                start_address = self.bus.load_prg(prg_file)
                if start_address is not None:
                    self.cpu.pc = start_address # Run the loaded program
            else:
                # Default startup
                self.cpu.pc = 0xFCE2 # Typical KERNAL cold start entry point
                print(f"C64 ROMs loaded. CPU PC set to C64 KERNAL reset vector: ${self.cpu.pc:04X}")
        ...
        while app_running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.VIDEORESIZE:
                    ...
                    if event.key == pygame.K_PAGEUP: # PageUp to scroll memory viewer up
                        if self.info_pages[self.current_info_page_index] == "Memory Viewer":
                            self.memory_view_addr = max(0x0000, self.memory_view_addr - 0x0100) # Scroll by 256 bytes
                    if event.key == pygame.K_PAGEDOWN: # PageDown to scroll memory viewer down
                        if self.info_pages[self.current_info_page_index] == "Memory Viewer":
                            self.memory_view_addr = min(0xFF00, self.memory_view_addr + 0x0100) # Scroll by 256 bytes
                    if event.key == pygame.K_F2: # F2 to toggle visualizers
                        self.show_visualizers = not self.show_visualizers

                    # Save/Load State Slots (F7-F12)
                    for i in range(NUM_SAVE_SLOTS):
                        key = pygame.K_F7 + i
                        if event.key == key:
                            slot_number = i + 1
                            filename = SAVE_STATE_FILENAMES[i]

                            # Check for Shift modifier
                            mods = pygame.key.get_mods()
                            if mods & pygame.KMOD_SHIFT:
                                # Shift+F<n> to Save State
                                self.save_emulator_state(filename)
                            else:
                                # F<n> to Load State
                                self.load_emulator_state(filename)

                    if event.key == pygame.K_F7:
                        mods = pygame.key.get_mods()
                        if mods & pygame.KMOD_SHIFT:
                            # Shift+F7 to Save State
                            self.save_emulator_state()
                        else:

                            # F7 to Load State
                            self.load_emulator_state()
                    if event.key == pygame.K_F11: # F11 to toggle turbo mode
                        self.turbo_mode = not self.turbo_mode
                    if event.key == pygame.K_F1: # F1 to show/hide help
                        self.show_help_screen = not self.show_help_screen
                    
                    # Handle C64 keyboard input
                    if event.key in C64_KEY_MAP:
                        row, col = C64_KEY_MAP[event.key]
                        self.bus.cia1.set_key_state(row, col, True)
                        # print(f"Key {pygame.key.name(event.key)} pressed (C64 Row {row}, Col {col})")
    ...
    def draw_info_panel(self, x_start_offset):
        ...
        if current_page_name == "CPU State":
            ...
            for i in range(20): # Show 20 instructions
                ...
                line = self.cpu.disassemble(temp_addr)
